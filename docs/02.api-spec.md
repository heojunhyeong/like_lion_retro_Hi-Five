...
- **Status Code:** 200 OK
- **Description:** 서버의 실시간 알림을 받기 위한 스트림 연결. 연결 후 서버에서 발생하는 각 이벤트 타입에 따라 브라우저 푸시 알림을 생성한다

```json
{
  "message": "알림 본문 텍스트",
  "url": "클릭 시 이동할 경로 (없을 경우 빈 문자열)"
}
```

| 이벤트 타입 | 수신 대상 | 발생 조건 | 이동 경로 (url) |
| :--- | :--- | :--- | :--- |
| **MATCH_REQUEST** | 방장 | 입장을 신청한 경우 | `/matches/{id}/manage` |
| **MATCH_ENTER** | 방장 | 방에 입장한 경우 | `/matches/{id}` |
| **MATCH_APPROVED** | 신청자 | 입장을 승인한 경우 | `/matches/{id}` |
| **MATCH_REJECTED** | 신청자 | 입장을 거절한 경우 | `""` (없음) |
